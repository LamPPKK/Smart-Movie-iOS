...
- **Connect TMDb safely** through browser approval or TV QR without entering a password in SmartMovie. Browse the authenticated profile and Account Details, browse paginated Movie/TV recommendations, rate Movie/TV/Episode titles from 0.5 to 10 in half-point steps, restore an Episode's existing remote score while pending local edits win, and manage account library/lists with durable offline mutation retry. Live signed-in device/TV validation remains tracked in [TMDb coverage](docs/TMDB_COVERAGE.md).
